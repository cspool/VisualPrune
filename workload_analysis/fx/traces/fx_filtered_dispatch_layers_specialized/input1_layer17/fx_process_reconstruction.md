...
怎么做/计算：`mul_tensor_1` 乘 input norm 权重；Q/K/V 三个分支分别 `view -> mm -> _unsafe_view`，使用 `_tensor_constant76/77/78`；再 `view` 成 `[1,624,32,128]`，`transpose` 到 `[1,32,624,128]`。
...
examples: Q[0,0,0,0], K[0,31,611,127], V[0,17,623,64].
```
...
怎么做/计算：`index_tensor/index_tensor_1` 用 `arg2_1` 从 `_tensor_constant79/80` 取 cos/sin；Q 分支计算 `transpose_int*cos`，再把 Dh 轴 `0..63` 和 `64..127` 切开，右半取负并拼回左半形成 rotate-half，乘 sin 后相加；K 分支同样生成 `add_tensor_2`。
...
怎么做/计算：`view_default_14` 展平 context；`mm_default_3` 乘 `_tensor_constant81`；`_unsafe_view_default_3` 恢复 `[1,624,4096]`；`add_tensor_4` 与 `arg0_1` 在相同 S/H 坐标逐元素相加。
...
examples: K[0,0,0], K[31,623,127], V[17,611,64].
```
