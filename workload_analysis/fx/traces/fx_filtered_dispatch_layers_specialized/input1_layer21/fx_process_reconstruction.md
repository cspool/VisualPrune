...
Trace directory: `workload_analysis/fx/traces/fx_filtered_dispatch_layers_specialized/input1_layer21`
...
mm_default = aten.mm.default(view_default, _tensor_constant114)
...
mm_default_1 = aten.mm.default(view_default_1, _tensor_constant115)
...
mm_default_2 = aten.mm.default(view_default_2, _tensor_constant116)
...
怎么做/计算：`mul_tensor_1` 乘 input norm 权重；Q/K/V 三个分支分别 `view -> mm -> _unsafe_view`，使用 `_tensor_constant114/115/116`；再 `view` 成 `[1,58,32,128]`，`transpose` 到 `[1,32,58,128]`。
...
examples: Q[0,0,0,0], K[0,31,57,127], V[0,21,57,64].
```
...
index_tensor = aten.index.Tensor(_tensor_constant117, [arg2_1])
unsqueeze_default = aten.unsqueeze.default(index_tensor, 1)
_tensor_constant118 = self._tensor_constant118
index_tensor_1 = aten.index.Tensor(_tensor_constant118, [arg2_1])
...
怎么做/计算：`index_tensor/index_tensor_1` 用 `arg2_1` 从 `_tensor_constant117/118` 取 cos/sin；Q 分支计算 `transpose_int*cos`，再把 Dh 轴 `0..63` 和 `64..127` 切开，右半取负并拼回左半形成 rotate-half，乘 sin 后相加；K 分支同样生成 `add_tensor_2`。
...
index_tensor_2 = aten.index.Tensor(view_default_13, [None, _tensor_constant119])
...
是什么：这一段按 `_tensor_constant119` 指定的 10 个 probe/index rows，从 last-query value contribution 中构造 `P x Hidden` 差分；该层不从这个差分继续产生新的返回 index，而是在 layer output 中透传 `arg6_1`。

为什么需要：它把 Visual/probe 相关 token 的 value-aware contribution 限定到较小的 10 行区域，便于和最后输出 row 比较。

怎么做/计算：`select_int` 从 `view_default_12` 选最后一个 sequence row；`select_int_1` 从 `clone_default` 选 `Q=-1` attention 权重；`unsqueeze_default_2` 扩展权重；`mul_tensor_6` 做 last-query weight 乘 V；`permute_default` 转 token-major；`clone_default_2` 连续化；`view_default_13` 得到 `[1,58,4096]`；`_tensor_constant119` 是 shape `[10]` 的 int64 index；`index_tensor_2` 从 token 轴选出 10 行；`unsqueeze_default_3` 扩展 last output row；`sub_tensor` 计算 reference minus selected probe rows。
...
S=0       +----------------------------------------+
          ...
          | rows selected by _tensor_constant119   |
...
mm_default_3 = aten.mm.default(view_default_14, _tensor_constant120)
...
怎么做/计算：`view_default_14` 展平 context；`mm_default_3` 乘 `_tensor_constant120`；`_unsafe_view_default_3` 恢复 `[1,58,4096]`；`add_tensor_4` 与 `arg0_1` 在相同 S/H 坐标逐元素相加。
...
mm_default_4 = aten.mm.default(view_default_15, _tensor_constant121)
...
mm_default_5 = aten.mm.default(view_default_16, _tensor_constant122)
...
mm_default_6 = aten.mm.default(view_default_17, _tensor_constant123)
...
examples: K[31,57,127], V[21,57,64], arg6_1[p].
```
...
| 18 | `qkv_projection` | `_tensor_constant114` | `get_attr` | `_tensor_constant114` | - | `mm_default` |
| 19 | `qkv_projection` | `mm_default` | `call_function` | `aten.mm.default` | `view_default`, `_tensor_constant114` | `_unsafe_view_default` |
...
| 22 | `qkv_projection` | `_tensor_constant115` | `get_attr` | `_tensor_constant115` | - | `mm_default_1` |
| 23 | `qkv_projection` | `mm_default_1` | `call_function` | `aten.mm.default` | `view_default_1`, `_tensor_constant115` | `_unsafe_view_default_1` |
...
| 26 | `qkv_projection` | `_tensor_constant116` | `get_attr` | `_tensor_constant116` | - | `mm_default_2` |
| 27 | `qkv_projection` | `mm_default_2` | `call_function` | `aten.mm.default` | `view_default_2`, `_tensor_constant116` | `_unsafe_view_default_2` |
...
| 35 | `rope` | `_tensor_constant117` | `get_attr` | `_tensor_constant117` | - | `index_tensor` |
| 36 | `rope` | `index_tensor` | `call_function` | `aten.index.Tensor` | `_tensor_constant117`, `arg2_1` | `unsqueeze_default` |
| 37 | `rope` | `unsqueeze_default` | `call_function` | `aten.unsqueeze.default` | `index_tensor` | `mul_tensor_2`, `mul_tensor_4` |
| 38 | `rope` | `_tensor_constant118` | `get_attr` | `_tensor_constant118` | - | `index_tensor_1` |
| 39 | `rope` | `index_tensor_1` | `call_function` | `aten.index.Tensor` | `_tensor_constant118`, `arg2_1` | `unsqueeze_default_1` |
...
| 83 | `visual_process` | `_tensor_constant119` | `get_attr` | `_tensor_constant119` | - | `index_tensor_2` |
| 84 | `visual_process` | `index_tensor_2` | `call_function` | `aten.index.Tensor` | `view_default_13`, `_tensor_constant119` | `sub_tensor` |
...
| 88 | `output_projection` | `_tensor_constant120` | `get_attr` | `_tensor_constant120` | - | `mm_default_3` |
| 89 | `output_projection` | `mm_default_3` | `call_function` | `aten.mm.default` | `view_default_14`, `_tensor_constant120` | `_unsafe_view_default_3` |
...
| 102 | `mlp` | `_tensor_constant121` | `get_attr` | `_tensor_constant121` | - | `mm_default_4` |
| 103 | `mlp` | `mm_default_4` | `call_function` | `aten.mm.default` | `view_default_15`, `_tensor_constant121` | `_unsafe_view_default_4` |
...
| 107 | `mlp` | `_tensor_constant122` | `get_attr` | `_tensor_constant122` | - | `mm_default_5` |
| 108 | `mlp` | `mm_default_5` | `call_function` | `aten.mm.default` | `view_default_16`, `_tensor_constant122` | `_unsafe_view_default_5` |
...
| 112 | `mlp` | `_tensor_constant123` | `get_attr` | `_tensor_constant123` | - | `mm_default_6` |
| 113 | `mlp` | `mm_default_6` | `call_function` | `aten.mm.default` | `view_default_17`, `_tensor_constant123` | `_unsafe_view_default_6` |
