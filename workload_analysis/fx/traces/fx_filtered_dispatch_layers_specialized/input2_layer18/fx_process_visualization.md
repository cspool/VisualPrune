...
- 怎么做/计算: #0 `arg0_1` 提供 `[1,1,4096]` hidden 给 #8 和 #83；#1 `arg1_1` 提供 `[1,1,1,625]` mask 给 #67；#2 `arg2_1` 供 #36/#39 RoPE 查表；其余 placeholder 在该图中不直接参与数值计算。
...
- 是什么: #55-#70 先把 cached K/V 与当前 K/V concat，再计算单 query 对 625 个 key 的 attention weights。
- 为什么需要: decode 时当前 query 只新增一个 token，但必须能读取历史 cache 中的所有 key/value。
- 怎么做/计算: #55 读取 cached K，#56 将 cached K 与 #54 当前 RoPE K 沿 token/cache 维 concat 为 `cat_default_2`，长度 625；#57 读取 cached V，#58 将 cached V 与 #34 当前 V concat 为 `cat_default_3`；#59 转置 concat K；#60/#61 view 当前 Q 为 `[32,1,128]`；#62/#63 view K^T 为 `[32,128,625]`；#64 `bmm` 得到 `[32,1,625]`；#65 view `[1,32,1,625]`；#66 scale；#67 加 mask；#68 softmax；#69 转 fp16；#70 clone。

```text
Decode attention: Q=1 attends over K_total=625

K_seq total
             cached keys 0..623                         current key 624
             ▲                                           ▲
concat K  ┌─────────────────────────────────────────────┬──────┐
          │ CACHED_KEY_ROWS                              │ CUR  │
          └─────────────────────────────────────────────┴──────┘
...
- 怎么做/计算: #71/#72 view weights 为 `[32,1,625]`；#73/#74 view concat V 为 `[32,625,128]`；#75 `bmm` 得到 `[32,1,128]`；#76 view；#77 transpose；#78 view 为 `[1,1,4096]`。
...
dynamic_cache.K    #56 cat_default_2             ──▶   [CACHE_KEY_ROWS: B=1,Heads=32,K=625,Dh=128]
dynamic_cache.V    #58 cat_default_3             ──▶   [CACHE_VALUE_ROWS: B=1,Heads=32,K=625,Dh=128]
