...
Trace directory: `workload_analysis/fx/traces/fx_filtered_dispatch_layers_specialized/input2_layer27`
...
arg1_1 mask [Q=1, K=59], axes Q=0..0 and K=0..58.
arg2_1 position id for the current token, token axis 0..0.
examples: hidden[0,0,0], mask[0,58], pos[0].
```
...
_tensor_constant257 = self._tensor_constant257
mm_default = aten.mm.default(view_default, _tensor_constant257)
_unsafe_view_default = aten._unsafe_view.default(mm_default, [1, 1, 4096])
view_default_1 = aten.view.default(mul_tensor_1, [1, 4096])
_tensor_constant258 = self._tensor_constant258
mm_default_1 = aten.mm.default(view_default_1, _tensor_constant258)
...
_tensor_constant259 = self._tensor_constant259
mm_default_2 = aten.mm.default(view_default_2, _tensor_constant259)
...
怎么做/计算：`mul_tensor_1` 乘 input norm 权重；Q/K/V 三个分支分别 `view -> mm -> _unsafe_view`，使用 `_tensor_constant257/258/259`；再 `view` 成 `[1,1,32,128]`，`transpose` 到 `[1,32,1,128]`。
...
examples: Q[0,0,0,0], K[0,31,0,127], V[0,27,0,64].
```

### RoPE position embedding

```python
_tensor_constant260 = self._tensor_constant260
index_tensor = aten.index.Tensor(_tensor_constant260, [arg2_1])
unsqueeze_default = aten.unsqueeze.default(index_tensor, 1)
_tensor_constant261 = self._tensor_constant261
index_tensor_1 = aten.index.Tensor(_tensor_constant261, [arg2_1])
...
怎么做/计算：`index_tensor/index_tensor_1` 用 `arg2_1` 从 `_tensor_constant260/261` 取 cos/sin；Q 分支计算 `transpose_int*cos`，把 Dh 轴 `0..63` 和 `64..127` 切开，右半取负并拼回左半形成 rotate-half，乘 sin 后相加；K 分支同样生成 `add_tensor_2`。
...
是什么：这一段把已有 K cache 与当前 K concat 成长度 59 的 K 轴，然后计算当前 query 对所有 keys 的 attention weights。

为什么需要：decode 时当前 token 要 attend 到历史 58 个 cache keys 和当前新 key；同时 `cat_default_2` 会作为更新后的 K cache 返回。

怎么做/计算：`_tensor_constant2` 是已有 K cache，`cat_default_2` 将它和当前 `add_tensor_2` 沿 token/cache 轴拼接；`_tensor_constant3` 是已有 V cache，`cat_default_3` 将它和当前 `transpose_int_2` 拼接；`transpose_int_3` 把 K 变成 `[Dh,K]`；Q/K 经 `expand/view` 成 `[32,1,128]` 和 `[32,128,59]`；`bmm_default` 得到 `[32,1,59]`；`view_default_8` 恢复 `[1,32,1,59]`；`div_tensor` 缩放；`add_tensor_3` 加 mask；`_softmax_default` 沿 K=59 归一化；`_to_copy_default_2` 转 fp16；`clone_default` 复制权重。

```text
Decode QK with cache concat

K axis after concat: 0 .. 58 = cached 0 .. 57 + current 58.
Q axis: 0 .. 0.

K cache / current K [K=59, Dh=128]
          Dh=0                                  Dh=127
K=0       +----------------------------------------+
          | cached key rows 0 .. 57                |
K=58      | current key add_tensor_2               |
          +----------------------------------------+

attention weights [Q=1, K=59]
          K=0                                  K=58
Q=0       +----------------------------------------+
          | softmax over cached keys + current key |
          +----------------------------------------+

examples: k_cache[0,0], k_current[58,127], weight[0,58].
```
...
是什么：这一段用当前 query 的 `[1,59]` attention weights 对 concat 后的 V cache 做加权求和，并合并多头 context。

为什么需要：当前 token 的 attention 输出要汇聚历史 value cache 和当前 value。

怎么做/计算：`clone_default` 展成 `[32,1,59]`；`cat_default_3` 展成 `[32,59,128]`；`bmm_default_1` 计算 `[1,K] x [K,Dh] -> [1,Dh]`；`view_default_11` 恢复 batch/head；`transpose_int_4` 转回 token-major；`view_default_12` 合并 heads 得到 `[1,1,4096]`。

```text
Attention-weighted V

weights [Q=1, K=59]        V [K=59, Dh=128]        context [Q=1, Dh=128]
K axis 0..58               Dh axis 0..127          Dh axis 0..127
Q=0  +------------------+  +-------------------+   +-------------------+
...
_tensor_constant262 = self._tensor_constant262
mm_default_3 = aten.mm.default(view_default_13, _tensor_constant262)
...
怎么做/计算：`view_default_13` 展平 context；`mm_default_3` 乘 `_tensor_constant262`；`_unsafe_view_default_3` 恢复 `[1,1,4096]`；`add_tensor_4` 与 `arg0_1` 在同一 H 坐标逐元素相加。
...
```python
_tensor_constant263 = self._tensor_constant263
mm_default_4 = aten.mm.default(view_default_14, _tensor_constant263)
...
_tensor_constant264 = self._tensor_constant264
mm_default_5 = aten.mm.default(view_default_15, _tensor_constant264)
...
_tensor_constant265 = self._tensor_constant265
mm_default_6 = aten.mm.default(view_default_16, _tensor_constant265)
...
dynamic_cache_layer:
K cat_default_2 [B=1, Heads=32, K=59, Dh=128]
V cat_default_3 [B=1, Heads=32, K=59, Dh=128]
examples: K[31,58,127], V[27,58,64], hidden[0,0,4095].
```
...
| 18 | `qkv_projection` | `_tensor_constant257` | `get_attr` | `_tensor_constant257` | - | `mm_default` |
| 19 | `qkv_projection` | `mm_default` | `call_function` | `aten.mm.default` | `view_default`, `_tensor_constant257` | `_unsafe_view_default` |
...
| 22 | `qkv_projection` | `_tensor_constant258` | `get_attr` | `_tensor_constant258` | - | `mm_default_1` |
| 23 | `qkv_projection` | `mm_default_1` | `call_function` | `aten.mm.default` | `view_default_1`, `_tensor_constant258` | `_unsafe_view_default_1` |
...
| 26 | `qkv_projection` | `_tensor_constant259` | `get_attr` | `_tensor_constant259` | - | `mm_default_2` |
| 27 | `qkv_projection` | `mm_default_2` | `call_function` | `aten.mm.default` | `view_default_2`, `_tensor_constant259` | `_unsafe_view_default_2` |
...
| 35 | `rope` | `_tensor_constant260` | `get_attr` | `_tensor_constant260` | - | `index_tensor` |
| 36 | `rope` | `index_tensor` | `call_function` | `aten.index.Tensor` | `_tensor_constant260`, `arg2_1` | `unsqueeze_default` |
| 37 | `rope` | `unsqueeze_default` | `call_function` | `aten.unsqueeze.default` | `index_tensor` | `mul_tensor_2`, `mul_tensor_4` |
| 38 | `rope` | `_tensor_constant261` | `get_attr` | `_tensor_constant261` | - | `index_tensor_1` |
| 39 | `rope` | `index_tensor_1` | `call_function` | `aten.index.Tensor` | `_tensor_constant261`, `arg2_1` | `unsqueeze_default_1` |
...
| 80 | `output_projection` | `_tensor_constant262` | `get_attr` | `_tensor_constant262` | - | `mm_default_3` |
| 81 | `output_projection` | `mm_default_3` | `call_function` | `aten.mm.default` | `view_default_13`, `_tensor_constant262` | `_unsafe_view_default_3` |
...
| 94 | `mlp` | `_tensor_constant263` | `get_attr` | `_tensor_constant263` | - | `mm_default_4` |
| 95 | `mlp` | `mm_default_4` | `call_function` | `aten.mm.default` | `view_default_14`, `_tensor_constant263` | `_unsafe_view_default_4` |
...
| 99 | `mlp` | `_tensor_constant264` | `get_attr` | `_tensor_constant264` | - | `mm_default_5` |
| 100 | `mlp` | `mm_default_5` | `call_function` | `aten.mm.default` | `view_default_15`, `_tensor_constant264` | `_unsafe_view_default_5` |
...
| 104 | `mlp` | `_tensor_constant265` | `get_attr` | `_tensor_constant265` | - | `mm_default_6` |
| 105 | `mlp` | `mm_default_6` | `call_function` | `aten.mm.default` | `view_default_16`, `_tensor_constant265` | `_unsafe_view_default_6` |
