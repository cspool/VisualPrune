...
该层的 Visual 相关过程是 middle probe：`t00000923` 与 `t00000922` 是按 Visual token 行和 Hidden 列对齐的两组表示；运行时先得到差值区域 `t00000924`，再沿 Hidden 维为每个 Visual token 生成相似度分数 `t00000926`，最后规约为布尔决策 `t00000929`。普通 Attn 输出路径只保留 evidence rows，不在这里画。
...
Visual token axis V=576 (compressed to 6 rows)           Hidden dimension
                                                            ...
                                                            │ CURRENT_VISUAL_ROWS                    │
                                                            │ CURRENT_VISUAL_ROWS                    │
current visual rows    t00000923  V=576               ──▶   │ CURRENT_VISUAL_ROWS                    │  ◀── 40:6 接近 4096:576
                                                            │ CURRENT_VISUAL_ROWS                    │
                                                            │ CURRENT_VISUAL_ROWS                    │
                                                            │ CURRENT_VISUAL_ROWS                    │
                                                            └────────────────────────────────────────┘
                                                            ...
                                                            │ REFERENCE_VISUAL_ROWS                  │
reference visual rows  t00000922  V=576               ──▶   │ REFERENCE_VISUAL_ROWS                  │  ◀── 与 current 同一 V×Hidden 坐标对齐
                                                            │ REFERENCE_VISUAL_ROWS                  │
                                                            │ REFERENCE_VISUAL_ROWS                  │
                                                            │ REFERENCE_VISUAL_ROWS                  │
                                                            └────────────────────────────────────────┘
                                                            ...
                                                            │ DELTA_COMPARE_ROWS                     │
                                                            │ DELTA_COMPARE_ROWS                     │
delta / compare rows   t00000924  V=576               ──▶   │ DELTA_COMPARE_ROWS                     │  ◀── current-reference 后沿 Hidden 维比较
                                                            │ DELTA_COMPARE_ROWS                     │
                                                            │ DELTA_COMPARE_ROWS                     │
                                                            │ DELTA_COMPARE_ROWS                     │
                                                            └────────────────────────────────────────┘
...
Visual score axis V=576 (same 6-row compression; 1-col expanded)
                                                            ...
                                                            │SCORE │
                                                            │SCORE │
score vector           t00000926  V=576               ──▶   │SCORE │  ◀── 每个 Visual token 行保留一个 score
                                                            │SCORE │
                                                            │SCORE │
                                                            │SCORE │
                                                            └──────┘
decision scalar        t00000929  shape=[]            ──▶   [REDUCED_BOOL_DECISION]                  ◀── 对 score vector 做 any 归约
```

Evidence: `#75 sub.Tensor` inputs=`[t00000923,t00000922]` output=`t00000924`, observed shape=`[1,576,4096]`; `#77 cosine_similarity.default` inputs=`[t00000924,t00000925]` output=`t00000926`, observed shape=`[1,576]`; `#80 any.default` input=`t00000928` output=`t00000929`, observed shape=`[]`.
