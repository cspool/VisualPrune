...
from .vp_flash_attention import vp_flash_attn_prefill
...
def validate_pruning_config(pruning_config: Dict, num_layers):
    if 'mode' not in pruning_config:
        raise ValueError("Pruning config must contain 'mode' key.")
    if not isinstance(pruning_config['mode'], list):
        raise ValueError("'mode' must be a list of strings.")
    for mode in pruning_config['mode']:
        if mode not in ['shallow', 'middle', 'deep']:
            raise ValueError(f"Unsupported pruning mode: {mode}. Supported pruning modes are combinations of 'shallow', 'middle', 'deep'.")

    if 'shallow' in pruning_config['mode'] or 'middle' in pruning_config['mode']:
        if 'shallow_mid_layer' not in pruning_config:
            raise ValueError("'shallow_mid_layer' must be specified when 'shallow' mode is used.")
        if not isinstance(pruning_config['shallow_mid_layer'], int) or pruning_config['shallow_mid_layer'] < 1 or pruning_config['shallow_mid_layer'] >= num_layers:
            raise ValueError(f"'shallow_mid_layer' must be an integer in the range of [1, {num_layers}].")

    if 'middle' in pruning_config['mode'] or 'deep' in pruning_config['mode']:
        if 'layer_threshold' in pruning_config and (not isinstance(pruning_config['layer_threshold'], float) or not (0 < pruning_config['layer_threshold'] < 1)):
            raise ValueError("'layer_threshold' must be a float between 0 and 1.")
        if 'tokens_threshold' in pruning_config and (not isinstance(pruning_config['tokens_threshold'], float) or not (0 < pruning_config['tokens_threshold'] < 1)):
            raise ValueError("'tokens_threshold' must be a float between 0 and 1.")
        
    if 'deep' in pruning_config['mode'] and 'middle' not in pruning_config['mode']:
        if "mid_deep_layer" not in pruning_config:
            raise ValueError("'mid_deep_layer' must be specified when 'deep' mode is used without 'middle' mode.") 
...
def get_model_size_from_path(config):
    model_path = getattr(config, '_name_or_path', None)
    if model_path is None:
        raise ValueError("Model path not found in config. Please make sure to pass 'model_name' in the config.")
    model_path = model_path.strip("/")
    model_paths = model_path.split("/")
    if '7b' not in model_paths[-1] and '13b' not in model_paths[-1]:
        raise ValueError("Model size not found in model path. Please make sure the model path contains '7b', '13b' or '70b'.")
    elif '7b' in model_paths[-1]:
        return '7b'
    elif '13b' in model_paths[-1]:
        return '13b'
...
class VisiPrunerLlamaAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        self.model_size = get_model_size_from_path(config)
        self.attention_dropout = config.attention_dropout
        self.hidden_size = config.hidden_size
        self.num_heads = config.num_attention_heads
        self.head_dim = self.hidden_size // self.num_heads
        ...
        self.rope_theta = config.rope_theta
        self.is_causal = True
        self.important_visual_tokens = None
        self.vis_end_index = 611
    ...
    def set_pruning_config(self, pruning_config: List[str]):
        self.pruning_mode = pruning_config.get('mode', None)
        self.shallow_mid_layer = pruning_config.get('shallow_mid_layer', 0)
        self.layer_threshold = pruning_config.get('layer_threshold', None)
        self.tokens_threshold = pruning_config.get('tokens_threshold', None)

    def value_aware_token_selection(self, value_states, attn_output, attn_weights, important_vis_tokens=None):
        ...
        if important_vis_tokens is None:
            ...
            if torch.any(cos_sim < self.layer_threshold).item() and important_vis_tokens is None:
    ...
    def set_num_images(self, num_images):
        ...
        self.vis_end_index = 35 + 576 * num_images
        self.vis_half_index = 35 + int((576 * num_images) / 2)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if important_vis_tokens is not None:
            return (attn_output, important_vis_tokens, exit_indicator), attn_weights, past_key_value
...
class FA2VisiPrunerLlamaAttention(VisiPrunerLlamaAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if q_len == 1 and "shallow" in pruning_mode:
            return super().forward(
                ...
                use_cache=use_cache,
                important_vis_tokens=important_vis_tokens,
        ...
        if q_len == 1 and past_key_value is not None and attention_mask is None:
            query_states_fa2 = query_states.transpose(1, 2)
            key_states_fa2 = key_states.transpose(1, 2)
            value_states_fa2 = value_states.transpose(1, 2)
        ...
        if need_pruning_proxy and pruning_weights is not None:
            ...
            attn_output_for_selection = (
                selection_attn_output_last
                if selection_attn_output_last is not None
                else attn_output
            )
            ...
            if ("middle" in pruning_mode and important_vis_tokens is None
                ...
                important_vis_tokens = self.value_aware_token_selection(
                    value_states_full, attn_output_for_selection, pruning_weights)
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        outputs += (important_vis_tokens, exit_indicator,)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def set_pruning_config(self, pruning_config: Dict):
        self.pruning_mode = pruning_config.get('mode', None)
        self.shallow_mid_layer = pruning_config.get('shallow_mid_layer', 0)
        self.layer_threshold = pruning_config.get('layer_threshold', None)
        self.tokens_threshold = pruning_config.get('tokens_threshold', None)
        ...
        for layer in self.layers:
            layer.self_attn.set_pruning_config(pruning_config)
    
    def get_important_vision_tokens(self, layer_idx):
        return getattr(self.layers[layer_idx].self_attn, "important_visual_tokens", None)
    ...
    def forward(
        ...
        pruning_mode = set(getattr(self, "pruning_mode", []) or [])
        use_visipruner_fa2_prefill_mask = (
            self._use_flash_attention_2
            and getattr(self, "num_images", 0) > 0
            and seq_length > 1
            and bool(pruning_mode.intersection({"middle", "deep"}))
        )
        use_visipruner_official_full_mask = (
            self._use_flash_attention_2
            and getattr(self, "num_images", 0) > 0
            and "shallow" in pruning_mode
        )

        if (
            self._use_flash_attention_2
            and not use_visipruner_fa2_prefill_mask
            and not use_visipruner_official_full_mask
        ):
        ...
        skip_visipruner_decode_bookkeeping = (
            self._use_flash_attention_2
            and complete_attn_mask is None
            and seq_length == 1
            and bool(pruning_mode.intersection({"middle", "deep"}))
            and "shallow" not in pruning_mode
        )
        # embed positions
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if self.num_images > 0 and not skip_visipruner_decode_bookkeeping:
                    ...
                    if hidden_states.shape[1] == 1 and ("middle" in getattr(self, "pruning_mode", []) or "deep" in getattr(self, "pruning_mode", [])) and layer_idx > getattr(self, "shallow_mid_layer", 80):
                        if complete_attn_mask is not None:
                    ...
                    if hidden_states.shape[1] > 1 and "middle" in getattr(self, "pruning_mode", []) and layer_idx > getattr(self, "shallow_mid_layer", 80) and hidden_states.shape[1] == inputs_embeds.shape[1]:
                        if important_vision_tokens != None:
                            ...
                            hidden_states = torch.cat((hidden_states[:,:35,:], important_vision_hidden_states, hidden_states[:,self.last_image_token_index:,:]), dim=1)
                            position_ids = torch.cat((position_ids[:,:35], position_ids[:,important_vision_tokens], position_ids[:,self.last_image_token_index:]), dim=1)
                        ...
                        if exit_indicator == 2:
                            hidden_states = torch.cat((hidden_states[:,:35,:], hidden_states[:,self.last_image_token_index:,:]), dim=1)
                            position_ids = torch.cat((position_ids[:,:35], position_ids[:,self.last_image_token_index:]), dim=1)
                            pruned_vis_len = hidden_states.shape[1]
                            attention_mask = complete_attn_mask[:,:,:pruned_vis_len,:pruned_vis_len]
                ...
                layer_outputs = decoder_layer(
                    ...
                    output_attentions=output_attentions,
                    use_cache=use_cache,
                    important_vis_tokens=important_vision_tokens,
                    exit_indicator=exit_indicator,
                )

                exit_indicator = layer_outputs[-1]
                important_vision_tokens = layer_outputs[-2]
...
class LlamaForCausalLM(CustomGenerationMixin, LlamaPreTrainedModel):
    ...
    def set_pruning_config(self, pruning_config: Dict):
        validate_pruning_config(pruning_config, self.num_layers)
        self.model.set_pruning_config(pruning_config)
